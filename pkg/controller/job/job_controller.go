...
func (jm *JobController) syncJob(key string) error {
	...
	if job.Spec.FailedPodsLimit != nil && failed > *job.Spec.FailedPodsLimit {
		...
		podsToDelete := podByCreationTimestamp(failedPods[howManyJobToKeep:]) // or by podByFailedTime(...) ?
		podsToDelete.Sort()
...
// podByCreationTimestamp sorts a list by creation timestamp, using their names as a tie breaker.
...
// Sort used to sort the pod by creationTimestamp
func (o *podByCreationTimestamp) Sort() {
	sort.Sort(o)
}
